...
**Status:** Implemented in Phase 1-3 roadmap (see docs/MASTER_PLAN.md)
**Decision Maker:** Developer (based on regulatory analysis below)

**Rationale:**
1. **Regulatory Compliance (5/5):** Classified as "marketing fee" not "referral fee" - minimal disclosure burden under ASCR Rule 12.4.3
2. **Fraud Prevention (4/5):** Booked calendar slots verify intent, mobile OTP prevents bots, optional booking deposits eliminate no-shows
3. **Lawyer Adoption (5/5):** Removes scheduling friction (15-30 min saved per lead), $100 fee negligible vs $400/hour billable rate
4. **Revenue Potential (4/5):** Higher price point ($100 vs $50 pay-per-lead), lower volume required for sustainability
5. **Implementation (3/5):** Medium complexity (calendar APIs, Stripe, OTP verification), but proven technology stack available

**Evolution Path:**
- **Phase 1-2 (Months 1-3):** FREE pilot to validate booking demand and lawyer conversion rates
  - Build calendar integration (Calendly/Google Calendar API)
  - Implement OTP verification (Twilio/MessageBird)
  - Track: booking completion rate (target >40%), consultation → client conversion (target >20%)
- **Phase 3 (Months 4-9):** Launch at $100 per booking with 10-20 firms
  - Implement Stripe payment infrastructure
  - Deploy fraud prevention stack (OTP, honeypot fields, IP fingerprinting)
  - Build lawyer dashboard (bookings, invoices, disputes)
  - Target: $48k-52k annual revenue
- **Phase 4 (Months 10+):** Transition to Hybrid Model (Option 5)
  - **Tier 1 - Premium Partner:** $149/month subscription + $50 per booking (50% discount)
  - **Tier 2 - Pay-As-You-Go:** $100 per booking (no monthly fee)
  - Benefits: Priority routing, market data access, recurring revenue (MRR)
  - Target: $155k-383k annual revenue

**Rejected Models:**
- **Option 4 (Pay-Per-Client-Signed):** FATAL FLAW - violates ASCR Rule 12 fee-sharing prohibition, requires disclosure "I paid $500 for your business" (erodes trust), attribution nightmare
- **Option 1 (Pay-Per-Lead):** High fraud risk (bots, fake forms), lawyer "tyre-kicker" fatigue, must spend 15-30 min on phone tag per lead
- **Option 2 (Pay-Per-Conversation):** Technical complexity (VoIP routing), lawyer availability friction (in court, can't take calls), revenue coupled to real-time availability

**Reference Implementation:**
- See: `docs/MASTER_PLAN.md` (lines 82-149) for revenue projections and business model flow
- See: `guides/phase1/CHECKLIST.md` for technical implementation tasks
- See: `docs/PHASE_1_2_UPDATES.md` for updated pricing model details and ROI calculations
- See: `guides/PRICING_IMPLEMENTATION_GUIDE.md` for step-by-step technical implementation (fraud prevention, Stripe integration, service agreements)

**Compliance Documentation:**
- **ASCR Rule 12.4.3:** Referral fee disclosure requirements → ✅ Simplified as "marketing and administrative fee"
- **Legal Profession Uniform Law Section 174:** Costs disclosure obligations → ✅ Compliant (included in lawyer's overhead/disbursements)
- **Fee-sharing prohibition (ASCR Rule 12, Uniform Law):** → ✅ Not triggered (payment for service, not percentage of legal fees)
- **Fiduciary duty (ASCR Rule 4.1.4):** → ✅ Lawyer maintains complete professional independence

**Unit Economics (Phase 3):**
```
Per Law Firm (Conservative):
- 4 bookings/month × $100 = $400/month revenue
- Lawyer billable: 4 × $400/hour = $1,600 consultation revenue
- Lawyer conversion: 1-2 clients × $3k-10k = $6,500 avg matter revenue
- Lawyer total revenue: $8,100
- Lawyer cost: $400 (CAC)
- Lawyer net profit: $7,700 (1,925% ROI)

Developer Revenue (10 firms):
- 40 bookings/month × $100 = $4,000/month
- Annual: $48,000/year
```

**Key Success Metrics:**
- Phase 2 (Pilot): >40% booking completion rate, >20% consultation → client conversion
- Phase 3 (Paid): >80% lawyer retention after 3 months, <20% no-show rate
- Phase 4 (Hybrid): >30% of firms upgrade to Premium Partner subscription

**Technical Stack:**
- Calendar: Calendly API or Google Calendar API
- Payments: Stripe (payment intents, subscriptions)
- Verification: Twilio Verify API (OTP), FingerprintJS (IP tracking)
- Fraud Prevention: Honeypot fields (passive), booking deposits (optional Stripe charge)
- Dashboard: Custom (React/Next.js with Stripe billing integration)
