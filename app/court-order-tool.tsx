...
import { ArrowLeft, Calendar, CheckCircle, FileText, Loader2, Upload } from 'lucide-react-native';
...
function StepResults({ result, onReset }: { result: CareCalculationResult; onReset: () => void }) {
  return (
...
export default function CourtOrderToolScreen() {
  ...
  const handleUpload = useCallback(() => {
    setIsUploading(true);
    setTimeout(() => { setIsUploading(false); setStep('anchor'); }, 2000);
  }, []);

  const handleCalculate = useCallback(() => {
    setStep('analyzing');
    setTimeout(() => {
      const calculationResult = calculateCareFromOrder(DUMMY_COURT_ORDER, anchorDate);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerSubtitle}>Calculate care percentages from your court order</Text>
      </View>
      <View style={styles.stepIndicator}>
        {(['Upload', 'Anchor Date', 'Analyzing', 'Results'] as const).map((label, index) => {
          const stepNames: WizardStep[] = ['upload', 'anchor', 'analyzing', 'results'];
      ...
      <View style={styles.content}>
        {step === 'upload' && <StepUpload onUpload={handleUpload} isUploading={isUploading} />}
        {step === 'anchor' && <StepAnchor anchorDate={anchorDate} onDateChange={setAnchorDate} onContinue={handleCalculate} onBack={() => setStep('upload')} />}
        {step === 'analyzing' && <StepAnalyzing />}
        {step === 'results' && result && <StepResults result={result} onReset={handleReset} />}
      </View>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  stepIndicator: { flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 20, paddingVertical: 16, backgroundColor: colors.surfaceSubtle, borderBottomWidth: 1, borderBottomColor: colors.border },
  stepIndicatorItem: { alignItems: 'center', flex: 1 },
  stepDot: { width: 24, height: 24, borderRadius: 12, backgroundColor: colors.border, alignItems: 'center', justifyContent: 'center', marginBottom: 4 },
  ...
  iconContainer: { width: 96, height: 96, borderRadius: 48, backgroundColor: colors.primaryLight, alignItems: 'center', justifyContent: 'center', marginBottom: 24 },
  loaderContainer: { width: 96, height: 96, borderRadius: 48, backgroundColor: colors.primaryLight, alignItems: 'center', justifyContent: 'center', marginBottom: 24 },
  stepTitle: { fontSize: 22, fontWeight: '700', color: colors.textPrimary, marginBottom: 12, textAlign: 'center' },
  stepDescription: { fontSize: 15, color: colors.textSecondary, textAlign: 'center', lineHeight: 22, marginBottom: 32, maxWidth: 400 },
  primaryButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: colors.primary, paddingVertical: 14, paddingHorizontal: 24, borderRadius: 12, gap: 8, minWidth: 200, ...shadowPresets.small },
  primaryButtonText: { color: '#ffffff', fontSize: 16, fontWeight: '600' },
  buttonDisabled: { opacity: 0.6 },
  secondaryButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surface, paddingVertical: 12, paddingHorizontal: 20, borderRadius: 12, borderWidth: 1, borderColor: colors.border, gap: 6 },
  ...
  datePickerButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.surface, paddingVertical: 12, paddingHorizontal: 16, borderRadius: 8, borderWidth: 1, borderColor: colors.border, gap: 12 },
  datePickerText: { fontSize: 16, color: colors.textPrimary },
  infoBox: { backgroundColor: colors.primaryLight, padding: 16, borderRadius: 12, maxWidth: 400, marginBottom: 32 },
  ...
  resultCard: { flex: 1, padding: 20, borderRadius: 16, alignItems: 'center', ...shadowPresets.medium },
  motherCard: { backgroundColor: '#fce7f3' },
  fatherCard: { backgroundColor: '#dbeafe' },
  resultLabel: { fontSize: 14, fontWeight: '600', color: colors.textSecondary, marginBottom: 8 },
  resultPercentage: { fontSize: 36, fontWeight: '700', color: colors.textPrimary },
  resultNights: { fontSize: 13, color: colors.textMuted, marginTop: 4 },
  holidaySection: { width: '100%', marginBottom: 24 },
  sectionTitle: { fontSize: 16, fontWeight: '600', color: colors.textPrimary, marginBottom: 12 },
  holidayList: { backgroundColor: colors.surface, borderRadius: 12, borderWidth: 1, borderColor: colors.border, overflow: 'hidden' },
  holidayItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 12, borderBottomWidth: 1, borderBottomColor: colors.borderSubtle },
  ...
  holidayBadgeText: { fontSize: 12, fontWeight: '600', color: colors.textSecondary },
  breakdownSection: { width: '100%', marginBottom: 24 },
  breakdownCard: { backgroundColor: colors.surface, borderRadius: 12, borderWidth: 1, borderColor: colors.border, padding: 16 },
  breakdownRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: colors.borderSubtle },
  ...
  percentageBar: { flexDirection: 'row', height: 24, borderRadius: 12, overflow: 'hidden', backgroundColor: colors.border },
  ...
  resetButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surface, paddingVertical: 12, paddingHorizontal: 20, borderRadius: 12, borderWidth: 1, borderColor: colors.border, gap: 6, marginTop: 8 },
});
